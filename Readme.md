...
From proxmox host (console rather than ssh), run:
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/ubuntu.sh)"

Create lxc advanced settings - unprivileged
4gb disk, 512mb memory, disable ipv6 (256mb memory not big enough to unpack chrome installation)


edit lxc container config file /etc/pve/lxc/[lxc id].conf  - add:
...
# -----------------------------
# 4️⃣ Fix /dev/shm for LXC
# -----------------------------
#mount -o remount,size=512M /dev/shm || true
#echo "Temporary /dev/shm increased to 512MB"

# -----------------------------
# 5️⃣ Create Selenium test script
# -----------------------------
...
Next, run python3 test_selenium.py    -  You should get "Page title: Google" returned


Next install extra pip packages for scraper:

pip install python-dotenv
pip install paho-mqtt

Now run python3 scrape-mqtt.py

Now setup cron job:
crontab -e   [select nano as your editor]

Paste the following at the end of your cron file:
